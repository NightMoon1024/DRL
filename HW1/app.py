...
def policy_evaluation():
    ...
    action_prob = 1.0 / len(action_list)

    policy = np.full((GRID_SIZE, GRID_SIZE), '', dtype='<U16')
    for i in range(GRID_SIZE):
        for j in range(GRID_SIZE):
            if (i, j) in obstacles or (i, j) == end_pos:
                continue
            # 每格只隨機選擇一個方向來當作箭頭顯示
            rand_action = np.random.choice(list(ACTION_SYMBOLS.keys()))
            policy[i, j] = ACTION_SYMBOLS[rand_action]
        ...
        for i in range(GRID_SIZE):
            for j in range(GRID_SIZE):
                if (i, j) in obstacles or (i, j) == end_pos:
                    continue
                v = 0
                for action, (di, dj) in action_list:
                    ni, nj = i + di, j + dj
                    if 0 <= ni < GRID_SIZE and 0 <= nj < GRID_SIZE and (ni, nj) not in obstacles:
                        v += action_prob * (REWARD + GAMMA * values[ni, nj])
                    else:
                        v += action_prob * (REWARD + GAMMA * values[i, j])
                new_values[i, j] = v
